...
function validateSizeandExt(field) {
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        if (files[i].size > allowedSize && allowedSize != 0) {
            warningString += "File is larger than the maximum file size of attachments.";
            warningString += "\n" + files[i].name + ": "  + getDisplaySize(files[i].size);
            warningString += " / ";
            warningString += "Maximum File Size: " + getDisplaySize(allowedSize);
            warningString += "\n";
...
function toggleEmptyState() {
    if (NOW.isUsingPolaris) {
        if (document.getElementById('no_available_attachments')) {
            document.getElementById('no_available_attachments').style.display = (hasAttachments()) ? 'none' : 'block';
        }
        document.getElementById('sys_attachment').style.visibility = (hasAttachments()) ? 'visible' : 'hidden';
    }
}
